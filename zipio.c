...
static int BufferAppend(struct ZipioState *zs, unsigned char *buffer,
                        long length) {
...
ZFILE *Zopen(const char *path, const char *mode) {
    ...
    if ((zs->sign != ZIPSIGNATURE) || (zs->flag & 1) || (zs->comp != 8) ||
        ...
        zs->inflatestate = InflateInitialize((void *)zs, inflate_putbuffer,
                                             inflate_malloc, inflate_free);
