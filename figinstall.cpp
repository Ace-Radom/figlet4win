...
const std::unordered_map<short, std::pair<std::string, std::string>>
    FIGFONT_ITEM_TYPE_NAME_MAP = {{FLF_T, {"FIGfont", ".flf"}},
                                  {FLC_T, {"FIGlet Control File", ".flc"}},
                                  {TLF_T, {"TOIlet Font", ".tlf"}}};

#ifndef NDEBUG

#define INFO(msg) std::cout << __FUNCTION__ << ": " << msg << std::endl;
...
#define ERROR(msg) std::cerr << __FUNCTION__ << ": " << msg << std::endl;
...
#define CURL_SETOPT_IF_FAILED_GOTO_CLEANUP(curl, curlcode, curloption, data) \
    ...
    if (curlcode != CURLE_OK) {                                              \
        ERROR("Failed to set " << #curloption << ": "                        \
...
int curl_write_string_callback(char* data, size_t size, size_t nmemb,
                               void* userdata) {
...
int curl_write_file_callback(char* data, size_t size, size_t nmemb,
                             void* userdata) {
...
std::string get_figfont_base_meta() {
    ...
    CURL_SETOPT_IF_FAILED_GOTO_CLEANUP(
        curl, res, CURLOPT_URL,
        "https://raw.githubusercontent.com/Ace-Radom/FIGfont_base/main/META");
    CURL_SETOPT_IF_FAILED_GOTO_CLEANUP(curl, res, CURLOPT_WRITEFUNCTION,
                                       curl_write_string_callback);
    CURL_SETOPT_IF_FAILED_GOTO_CLEANUP(curl, res, CURLOPT_WRITEDATA, &content);
    CURL_SETOPT_IF_FAILED_GOTO_CLEANUP(curl, res, CURLOPT_SSL_VERIFYPEER,
                                       false);
    res = curl_easy_perform(curl);
    if (res != CURLE_OK) {
        ERROR(
            "Failed to connect to GitHub gateway: " << curl_easy_strerror(res));
        goto CLEANUP_SIGN;
    }
...
figfont_list_t parse_figfont_base_meta(std::string meta) {
    ...
    while (std::getline(iss, line)) {
        if (line == "#FIGfonts") {
            ...
            std::string full = line.substr(line.find(':') + 1,
...
int list_remote_fonts(std::string prefix) {
    ...
    std::cout << "Font base metadata got, " << font_list.size()
    ...
    for (; i < font_list.size() && font_list[i].type == FLF_T; i++) {
        if (!prefix.empty() &&
            font_list[i].shorten.find(prefix) == std::string::npos &&
            font_list[i].full.find(prefix) == std::string::npos) {
            continue;
        }
        std::cout << " * " << font_list[i].shorten << ": " << font_list[i].full
                  << std::endl;
    }
...
std::filesystem::path download_remote_font_to_temp(const figfont_node_t& item) {
    ...
    oss << "https://raw.githubusercontent.com/Ace-Radom/FIGfont_base/main/"
        << item.full << FIGFONT_ITEM_TYPE_NAME_MAP.at(item.type).second;
    url = oss.str();
    ...
    temp_file_path = temp_file_path / "FIGlet4Win" / "fonts" /
                     (item.full.substr(0, item.full.find('/')));
    if (!std::filesystem::exists(temp_file_path)) {
        std::filesystem::create_directories(temp_file_path);
    }
    temp_file_path /= (item.full.substr(item.full.find('/') + 1) +
                       FIGFONT_ITEM_TYPE_NAME_MAP.at(item.type).second);
...
void print_helpmsg() {
    std::cout
        << "Usage: figinstall [ -?Ih ] | [ [ -L | -l ] | [ -rR ] ] [ font ]"
        << std::endl;
    return;
}
...
void print_infomsg() {
    std::cout << "FIGinstall, FIGlet Font Installer" << std::endl
              ...
              << "Version: " << FIGLET4WINVERSION
              << ", date: " << FIGLET4WINBUILDTIME << std::endl
              << "FIGlet4Win, along with the various FIGlet fonts and "
              ...
              << "Contributes, bug reports and feature requests to FIGlet4Win "
                 "are welcome."
              << std::endl
              << "Repo-site: <https://github.com/Ace-Radom/figlet4win>"
              << std::endl;
    return;
}
...
int main(int argc, char** argv) {
    ...
    if (remote_font) {
        ...
        if (match_list.size() > 1) {
            ...
            for (int i = 0; i < match_list.size(); i++) {
                std::cout
                    << i + 1 << ". " << std::left << std::setw(24)
                    << FIGFONT_ITEM_TYPE_NAME_MAP.at(match_list[i].type).first
                    << match_list[i].shorten << ":" << match_list[i].full
                    << std::endl;
            }
            while (1) {
                ...
                if (in >= 1 && in <= match_list.size()) {
                    install_target =
                        download_remote_font_to_temp(match_list[in - 1]);
    ...
    if (restrictive_mode) {
        ...
        chkfont_cmd = std::string("\"") + chkfont_path.string() + "\" " +
                      install_target.string();
        // `"xxx\chkfont.exe" xxx.flf`
        chkfont_cmd_w =
            std::wstring_convert<std::codecvt_utf8<wchar_t>>().from_bytes(
                chkfont_cmd);
        conv_req_size = WideCharToMultiByte(CP_ACP, 0, chkfont_cmd_w.c_str(),
                                            -1, NULL, 0, NULL, NULL);
        chkfont_cmd.clear();
        WideCharToMultiByte(CP_ACP, 0, chkfont_cmd_w.c_str(), -1,
                            &chkfont_cmd[0], conv_req_size, NULL, NULL);
    ...
    install_to_path = install_to_path.parent_path() / DEFAULTFONTDIR /
    ...
    // it cannot overwrite existing targets even if
    // fs::copy_options::overwrite_existing is set
