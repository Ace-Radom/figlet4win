...
void readchar() {
    ...
    for (i = 0; i < charheight; i++) {
        ...
        if (i == 0) {
            ...
            if (endmark != expected_endmark && !incon_endmarkwarn) {
                printf("%s: Warning- Inconsistent endmark in line %d.\n",
                       fontfilename, currline);
                printf(
                    "%s:          (Above warning will only be printed once.)\n",
                    fontfilename);
                incon_endmarkwarn = 1;
                wc++;
                weregone(0);
        ...
        if (diff > 2) {
            ...
            if (!endmark_countwarn) {
                printf(
                    "%s: Warning- Endchar count convention violated in line "
                    "%d.\n",
                    fontfilename, currline);
                printf(
                    "%s:          (Above warning will only be printed once.)\n",
    ...
    if (minleadblanks + mintrailblanks > 0 && old_layout >= 0) {
        printf(
            "%s: Warning- Leading/trailing blanks in char. ending at line "
            "%d.\n",
            fontfilename, currline);
        printf(
            "%s:          (Above warning only given when old_layout > -1.)\n",
            fontfilename);
        wc++;
        weregone(0);
        if (gone) return;
...
void checkit() {
    ...
    numsread = sscanf(fileline, "%c %d %d %d %d %d %d %d %d", &hardblank,
                      &charheight, &upheight, &maxlen, &old_layout, &cmtcount,
    ...
    while (fgets(fileline, maxlen + 1000, fontfile) != NULL) {
        ...
        if (theord < 127 && theord > 31 && !asciicodetagwarn) {
            ...
            weregone(0);
            if (gone) return;
        } else if (theord <= oldord && theord >= 0 && oldord >= 0 &&
                   !nonincrwarn) {
